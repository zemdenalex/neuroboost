...
func helpText(lang i18n.Lang, screen string) string {
	...
	case keyboards.HelpDayTasks:
		return i18n.T(lang,
			"ℹ️ <b>📌 Задачи дня</b>\n\nНесколько дел, которые ты берёшь на день. День красится по тому, сколько из них закрыто в этот день: ⬛ ничего · 🟫 · 🟥 · 🟧 · 🟨 · 🟩 всё.\n\nСколько дел в дне, выбирается в настройках (от 3 до 7). Прошедший день, который не взяли, будет ⬛. Сегодня красится, когда день взят. Дни до первого взятого дня не красятся, если в настройках не выбрано иное.\n\nЗадачи дня можно выключить в настройках: тогда они пропадут из меню, карточек и календаря, а взятое сохранится. Дни, пока они были выключены, считаются невзятыми и тоже будут ⬛.\n\n⬜ сегодня нажатием отмечается сделанным. Убрать задачу из сегодняшнего дня можно до 12:00, добавить можно всегда.\n\n<b>Отменить:</b> убери задачу через ✏️ Поменять до полудня. Прошедший день не меняется.",
			"ℹ️ <b>📌 Day tasks</b>\n\nA few things you take on for a day. The day is coloured by how many of them are closed that day: ⬛ none · 🟫 · 🟥 · 🟧 · 🟨 · 🟩 all.\n\nHow many tasks a day holds is set in settings (3 to 7). A past day that was not taken is ⬛. Today gets its colour once the day is taken. Days before the first day taken are not coloured unless settings say so.\n\nDay tasks can be switched off in settings: they leave the menu, the cards and the calendar, and what was taken is kept. Days while they were off count as not taken and are ⬛ too.\n\n⬜ today is marked done with a press. A task can leave today until 12:00; adding is always allowed.\n\n<b>Undo:</b> take the task out with ✏️ Change before noon. A past day does not change.")
	case keyboards.HelpSubtasks:
		return i18n.T(lang,
			"ℹ️ <b>➕ Подзадачи</b>\n\nБольшую задачу можно разбить на шаги. Шаги видны на её карточке: ⬜ нажатием отмечается сделанным, ✅ открывает шаг.\n\nЗадача с подзадачами показывает, сколько шагов сделано. Подзадача попадает в тот же календарь, что и задача, и видна в общих списках как обычная задача.\n\n<b>Отменить:</b> ❌ Отмена закрывает вопрос; лишнюю подзадачу можно удалить с её карточки.",
			"ℹ️ <b>➕ Subtasks</b>\n\nA big task can be split into steps. The steps are on its card: ⬜ marks one done with a press, ✅ opens it.\n\nA task with subtasks shows how many steps are done. A subtask goes into the same calendar as its task and appears in the lists like any task.\n\n<b>Undo:</b> ❌ Cancel closes the question; a subtask you do not need can be deleted from its card.")
