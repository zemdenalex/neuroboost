...
- [[decision-bottom-tabs-three-26-09]] — "Денис 26.09: нижняя панель на телефоне (веб и Mini App) — только «Календарь · Добавить · Ещё»; 3 или 5 вкладок максимум и симметрично, Agenda / Задачи / Настройки уходят в «Ещё»"
- [[decision-web-home-is-the-bots-today-26-09]] — "Денис 26.09: главная веба как «📅 Сегодня» бота — строка задач дня с кнопкой, события с–до, пять задач с галочкой и «и ещё N», счётчики ниже"
- [[decision-language-web-and-miniapp-shared-bot-own-26-09]] — "Денис 26.09 (handoff): язык общий у веба и Mini App; бот и будущее Android-приложение держат свой — утреннее «один язык на человека» пересмотрено"
- [[decision-one-line-parser-is-the-bots-26-09]] — "Денис 26.09: ввод одной строкой в вебе разбирается тем же парсером, что в боте, через API (вариант A), а не портом на TypeScript"
- [[decision-phone-month-three-variants-26-09]] — "Денис 26.09: месяц на телефоне — строить три варианта, человек выбирает сам: A месяц + список дня, C полоса недели, D тепловая карта"
- [[decision-release-11-6-then-12-0-26-09]] — "Денис 26.09: релизы по одному — сначала v0.4.11.6 (PR #10, бот как есть), потом v0.4.12.0 (веб + API + бот с develop)"
- [[decision-task-event-link-sheet-and-steps-26-09]] — "Денис 26.09: связать/перенести задачу и событие в вебе — лист из карточки (A) с шагами как в боте (B), где нужен выбор серии"
- [[decision-web-follows-bot-priority-symbol-26-09]] — "Денис 26.09: веб рисует приоритет в стиле, выбранном в боте (кружки / ●1 ○3 / тире), и может его менять — одна настройка settings.bot.priority_style"
...
- [[learning-a-page-showing-today-must-name-today]] — "Страница, которая показывает «сегодня», обязана сама слать дату: без неё сервер выбирает «день серии» (PressedDay), и галочка в среду закрывает пятницу"
- [[learning-a-series-due-date-is-its-start]] — "У повторяющейся задачи due_date — день начала серии: любая проверка «просрочено» обязана пропускать rrule, иначе ежедневная серия просрочена навсегда"
...
- [[learning-local-e2e-flakes-on-the-dev-server-not-the-code]] — "Локальный e2e на vite dev-сервере флакает на первой загрузке (2–4 из 11, каждый раз разные); те же спеки против vite build + preview — 11/11 за 43 с"
- [[learning-checklist-lines-go-stale-check-git-first]] — "Строка чек-листа протухает: прежде чем брать пункт, искать в git коммит, который его уже закрыл — за ночь 26.09 таких нашлось шесть"
- [[learning-a-default-before-on-conflict-resets-unnamed-fields]] — "Upsert, который подставляет значение по умолчанию ДО ON CONFLICT, перезаписывает поля, не названные в запросе: дефолт решать отдельно для INSERT и для UPDATE"
...
- [[learning-a-question-to-denis-is-not-a-pause]] — "Заданный Денису вопрос — не пауза лупа: пока он не ответил, делается соседний пункт (страница вариантов, другая строка); 26.09 луп простоял 09:48–12:55 в ожидании ответа"
- [[learning-a-push-carries-every-commit-on-the-branch]] — "push ветки уносит все её коммиты, в том числе коммиты работающего рядом агента, ещё не проверенные — пушить после его отчёта или из отдельной ветки"
...
- [[learning-govulncheck-measures-the-local-toolchain]] — "govulncheck меряет локальный тулчейн и достижимость по графу вызовов, а не образ и не конфигурацию: прод собирался на Go 1.22, а pgx-уязвимость была недостижима по настройке"
- [[learning-measure-develop-before-blaming-staging]] — "Замер staging показывает старый код: CLS /tasks 0.121 на staging и 0.029 на develop локально против той же базы — прежде чем чинить, мерить develop"
- [[learning-equal-z-index-lets-dom-order-decide]] — "Два fixed-слоя с одинаковым z-index: сверху тот, что позже в DOM — локально лист был над нижней панелью, на CI панель забирала нажатия"
- [[learning-playwright-call-log-prints-the-bearer-token]] — "Call log Playwright печатает заголовки запроса, в том числе Authorization: Bearer — вывод e2e читать через фильтр"
...
- [[preference-loop-goal-before-standing-work]] — "Луп работает на свою названную цель; побочные мелкие задачи (починки, документы, исследование, свои инструменты) — когда застрял на цели или следующий шаг цели огромный (большой рефакторинг, миграция), потом обратно к цели"
