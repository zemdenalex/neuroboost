...
## [2026-09-20] recall | learning-the-right-time-in-the-wrong-zone, learning-an-empty-result-is-not-an-answer, learning-e2e-baseline-recorded-on-a-monday, learning-a-test-that-cannot-fail-guards-nothing, learning-green-tests-are-not-a-deployed-bot, learning-a-button-is-not-a-feature, learning-my-first-cache-test-passed-with-the-cache-off, decision-bot-vocabulary-and-symbols-18-09

## [2026-09-20] continuation — следующая сессия: доводка по фидбеку Дениса

**Фокус, его словами:** «we'll continue with improving on my feedback in the next session».
Действующий чеклист — `docs/proverka-bota-v0.4.11.4-2026-09-20.md`, его ответы дословно —
`ref/feedback/bot-proverka-v04114-otvet-denisa-2026-09-20.md`.

**Что случилось за сессию.** Денис прошёл чеклист v0.4.11.4 и встал на разделе 2: «не могу
создать повторяющуюся задачу». Он был прав, и причина глубже кнопки — у API **не было поля
`rrule`** ни в create, ни в update, а ночные тесты сеяли колонку прямым INSERT'ом. Фича была
в релизных заметках, покрыта тестами и не существовала. Узел:
`learning-a-test-that-seeds-the-column-never-opened-the-door`.

Починено и выкачено на dev (бот — руками, коммит `aa0bef5`; API — CI, миграции 21, не dirty):
- API принимает `rrule`/`nag_minutes` на создании и правке, правило валидируется тем же
  `recurrence.Parse`, что читает его потом; на update едет внутри запроса `updateTask`, иначе
  прошло бы мимо `calendar_id = ANY(writable)` (`5c8266f`)
- `recurrence.dayOf` сравнивал **моменты вместо дат** — у москвича «каждый понедельник»
  всплывало бы по вторникам (`learning-a-day-is-a-date-not-an-instant`)
- бот: вокабуляр повтора у задач, строка 🔁 на карточке, шаг мастера, **текстовые ответы**
  на шагах («5м», «раз в 3 дня») — это его «почему здесь нет своего варианта» (`97e9e22`)
- бот: «✅ На сегодня» вместо «Готово» у серии (раньше `status=DONE` убивал серию навсегда),
  ⏰ Отложить день/2/3/неделя/месяц через `postpone_days` (`aa0bef5`)
- v0.4.11.5 (связка аккаунтов) написана целиком — API, бот, веб; на стенде её нет

**Что открыто, по убыванию его интереса:**
1. 🔴 **Статистика** — `decision-statistics-is-a-screen-you-browse-20-09`. Не начато. Два
   вопроса ему заданы и **без ответа**: объединять ли пересечения в «Занято» и чем заменить
   `▪`. Начинать с ответов, а не с кода.
2. 🔴 **Задача ↔ событие** (раздел 4 чеклиста) — эндпоинт `/convert` есть с 18.09, кнопки нет.
   Его форма — «1 + 3»: спросить перенести-или-связать, показать какое поле чем станет,
   спросить недостающее.
3. 🔴 Выключить повтор у существующей задачи из бота (API умеет, кнопки нет).
4. 🔴 Частота напоминаний в настройках бота (`nag_minutes` есть, экрана нет).
5. 🔴 Правка одного вхождения повтора — пункт 12, не начат.
6. 🔴 Мобильные экраны веба — долг с 18.09, не трогались.
7. 🔴 Мерж в `main` — только по его явному «да». **До мержа snooze на проде сломан.**

**Читать первыми:** `learning-a-test-that-seeds-the-column-never-opened-the-door` ·
`learning-a-day-is-a-date-not-an-instant` · `learning-a-cached-test-result-is-a-test-that-did-not-run` ·
`decision-statistics-is-a-screen-you-browse-20-09` · `learning-green-tests-are-not-a-deployed-bot`.

**Рабочие заметки для следующего раза:**
- 🔴 Тестовая БД — одноразовый контейнер `postgres:16` на порту 55432, имя `nb-test-db`;
  пользователь и база `neuroboost` / `neuroboost_test`, пароль задаётся при запуске и живёт
  только в нём. Команду целиком держать в `memory/`, не здесь: **строка подключения с паролем
  в репозитории — это учётные данные в репозитории**, и push-гейт справедливо это поймал
  20.09. Гонять всегда `-count=1` и с заданным `DATABASE_URL`: **без переменной половина
  тестов молча скипается, с кэшем — врёт.**
- 🔴 Docker Desktop после перезагрузки не поднят сам: запускать и ждать `docker info`.
- 🔴 Правки Go-файлов — только Write/Edit. Heredoc дважды за сессию превратил `\n` внутри
  строк в настоящий перенос и сломал файл; один раз это стоило `git checkout` и переделки.
- ⚠ `gh run view --json conclusion` у незавершённого прогона отдаёт пустую строку, а не
  «in_progress» — цикл ожидания писать на непустоту, иначе он выходит сразу.

**Навыки на следующую сессию:** `tdd` (вертикальный срез до кнопки, как в этой),
`superpowers:brainstorming` перед статистикой — у неё два измерения и это разговор, а не
задача; `handoff` в конце.

## [2026-09-20 21:23] consolidate | PRESERVED — transcript-20260920-212049.jsonl, 77 MB → .oversize/ (too large for one pass, kept for chunked consolidation)

## [2026-09-21] recall | decision-statistics-is-a-screen-you-browse-20-09, learning-a-cached-test-result-is-a-test-that-did-not-run, learning-a-day-is-a-date-not-an-instant

## [2026-09-21] handoff | v0.4.11.3 на проде; 11.4 собирается на dev

**Сделано за сессию:**
- 🟢 **v0.4.11.3 на проде** (`c86e581`, тег): срез по 18.09, миграции 21 не dirty, данные
  целы, прод-бот руками. **Snooze на проде починен.** Запись — `docs/relizy/plan-reliza-v0.4.11.3-2026-09-21.md`
- Рассылка «что нового» — 2 из 3 (англоязычный получатель: 403 оба раза)
- Авария площадки М9, 15:52–17:07 MSK, прод и dev вместе — `docs/incident-2026-09-21-host-nedostupen.md`
- Проход Дениса №2 по 11.4 разобран: 10 дефектов с `file:line` — `docs/razbor-prohoda-v04114-2026-09-21.md`
- На dev (не на проде): `PressedDay` · ошибки словами вместо JSON · запятые спрашивают ·
  «спрошу» спрашивает · «Сегодня» по-русски с временем «до» и честным счётчиком · задачи в дне
  календаря · 🔁 Повтор / 🔔 Долбить / ✏️ Своё на карточке · подсказки в мастере

**Следующая сессия — v0.4.11.4 по плану `docs/superpowers/plans/2026-09-21-v04114-release-plan.md`:**
1. **A. Задача ↔ событие** — его «1 + 3», `/convert` есть. 🟡 спросить: при «связать» закрывает
   ли отметка события задачу
2. **B. Символ приоритета** — 3 варианта в настройках + шаг онбординга, скан-тест на обход
3. Денис проходит чеклист dev → мерж в `main` только по его «да». Миграций в 11.4 нет
4. Потом, в его порядке: **отписка → статистика → задачи дня**

**Читать первыми:** `decision-release-small-and-in-his-order-21-09` ·
`learning-count-the-failures-not-the-successes` · `decision-priority-style-is-a-choice-21-09` ·
`learning-one-defect-can-hide-another` · `decision-statistics-is-a-screen-you-browse-20-09`

**Навыки:** `superpowers:writing-plans` на каждый кусок (A, B) перед кодом;
`superpowers:test-driven-development`; перед релизом — план в `docs/relizy/`.

⚠ В рабочем дереве `docs/proverka-bota-v0.4.11.4-2026-09-20.html` — не мой, не трогал.

## [2026-09-21] handoff-amend | состав 11.4 уточнён спекой после продолжения выше

🔴 **Продолжение выше устарело в двух пунктах — читать это:**
- Работа следующей сессии идёт по **спеке** `docs/superpowers/specs/2026-09-21-v04114-bot-design.md`
  (статус: на ревью у Дениса), черновой план — только фон.
- Состав 11.4: dev + **A** задача↔событие **в обе стороны** (новый `POST /api/events/{id}/to-task`,
  `repeat: series|once`, 🔴 часовой пояс события сейчас захардкожен Москвой) + **B** символ
  приоритета + **C** «ℹ️ Что это?» на новых экранах + **D** отписка (переехала в 11.4: сам 11.4
