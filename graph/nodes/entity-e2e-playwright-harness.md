...
weight: { importance: 4, connectivity: 16, access: 5, last_accessed: 2026-09-11 }
sources:
...
**Текущая линия — 23 passed, 3 skipped** (11.08). Спеки: `smoke`, `auth-fixture`,
...
🔴 **3 skipped — это три drag-спеки на mobile**: `multiday-resize`, `crossday-resize`,
`move-grab-offset`. Причина структурная: на 375px календарь показывает один день, соседней
колонки просто нет на экране.

⚠️ **Причину пропуска проверять, а не придумывать.** `resize-click-noop` сначала пропускал
mobile с формулировкой «handle рассчитан на курсор» — догадка. Проверил: проходит. Пропуск
снят, покрытие выросло. Пропуск с правдоподобной, но непроверенной причиной читается следующей
сессией как проверенный.
...
⚠️ **Drag-спеки намеренно работают в 03:00–05:00 по местному времени**, а не в рабочие часы.
Причина не в стиле: у Дениса в календаре живые события, и первая версия спеки промахнулась
мимо своего блока в соседнее — повторяющееся — и открыла диалог «Только это событие / Все
повторы», из-за чего падение читалось как сломанный resize. Ранний час оставляет колонку
пустой и блок — во всю ширину. **Не «прибирать» их в рабочие часы.**
