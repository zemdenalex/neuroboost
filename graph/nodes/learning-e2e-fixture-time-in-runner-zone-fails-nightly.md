...
## 🔴 22.09, 02:58 МСК — третий раз, в том же пакете, мимо той защиты

`TestATaskWhoseEventHasPassedRemindsAgain` (CI `d0aa668`, 23:58 UTC): задача со сроком «завтра
00:00 UTC» и напоминанием за 10 мин → напоминание в **23:50 сегодня**; скан идёт от «сейчас» →
в окне 23:50–00:00 UTC его нет, тест про другое падает. `time.Local = time.UTC` тут не помогал:
зона была верной, **час прогона** — нет. Лечение — `twoMidnightsAhead()` в `convert_db_test.go`
(срок через две полуночи: напоминание всегда 24–48 ч впереди). Сабботаж «срок через 5 минут»
воспроизвёл ровно сообщение CI. Вопрос, который находит этот класс: **«в какой час прогона
фикстура уже в прошлом?»**
