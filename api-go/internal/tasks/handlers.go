...
func listTasks(ctx context.Context, userID, status, category, taskContext string) ([]Task, error) {
	...
	query := `
		SELECT t.id, t.calendar_id::text, t.user_id, t.title, t.description, t.status, t.category,
		       ...
		       (SELECT e.id::text FROM event e WHERE e.task_id = t.id AND e.calendar_id = ANY($1) LIMIT 1),
		       COALESCE(o.state, '')
		FROM task t
...
func getTask(ctx context.Context, userID, taskID string) (*Task, error) {
	...
	err = db.Pool.QueryRow(ctx, `
		SELECT t.id, t.calendar_id::text, t.user_id, t.title, t.description, t.status, t.category,
		       ...
		       t.updated_at, t.actual_minutes, COALESCE(t.reminder_offsets, '{}'),
		       t.rrule, t.repeat_anchor, t.nag_minutes,
		       (SELECT e.id::text FROM event e WHERE e.task_id = t.id AND e.calendar_id = ANY($2) LIMIT 1),
		       COALESCE(o.state, '')
		FROM task t
		LEFT JOIN task_occurrence o
		       ON o.task_id = t.id
