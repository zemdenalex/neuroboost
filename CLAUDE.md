...
- **MD1 — resize не уходит на соседний день** (Medium). `onMove` не переносит X курсора в
  состояние resize, поэтому конец нельзя перетащить в другую колонку. **Оставлено намеренно:**
  сначала нужно обобщить ghost, иначе неаккуратное вертикальное движение утащит событие в
  соседние сутки — баг хуже исправляемого. Корень тот же: `utcToLocalMinutes`
  (`timezone.utils.ts:38-43`) делает `localMs % DAY_MS` и выбрасывает дату.
