...
своё ограничение, читается как действующий запрет: см. `learning-stale-comment-outlived-its-constraint`.
  ...
  **настоящим перетаскиванием мыши** — `web/e2e/multiday-resize.spec.ts`, утверждение по базе,
  ...
  ⚠ **Уточнение, стоившее ложного диагноза (10.08):** порога движения у resize действительно нет
  (у `move` он есть — 5px через `pending`), но **простой клик событие НЕ схлопывает** — это
  описание старой min/max-версии. Клик отправляет **PATCH с неизменёнными временами**; у
  повторяющегося события он вдобавок открывает диалог «Только это событие / Все повторы» на ровном
  месте. Раздражает, данные не портит. Пришпилено тестами `dragHandlers.test.ts` («resize that
  never moved»), чтобы схлопывание не вернулось незаметно.
