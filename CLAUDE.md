...
14. 🔴 **Логи бота печатают его токен, и это делает не только библиотека.** `docker logs
    ...
    **любое** чтение логов отправляет секрет в транскрипт. Читать только через фильтр:
    ...
    ⚠ **Свой код течёт тем же способом:** `%v` от `*url.Error` несёт полный URL с токеном —
    `bot/internal/notifier/notifier.go:73` печатает его на **ожидаемом** пути отказа (юзер
    заблокировал бота), а pull тикает постоянно. Фильтр спасает читателя, но не файл на диске:
    **токен подлежит ротации.**
