...
- **MD1 / MD2 — move и resize многодневного события** (High). Корень — **система координат**, а не
  ...
  игнорирует X курсора (`useWeekGridDrag.ts:106`).

  🔴 **Поля `anchorMs` / `cursorMs` — мёртвый код.** `handleResizeComplete` их читает
  (`dragHandlers.ts:96-97`), но `startResizeStart` / `startResizeEnd` их **не задают** нигде,
  поэтому всегда работает day-relative fallback. Пока их не начнут писать, абсолютный путь
  ничего не чинит. Проверка: `grep -rn "anchorMs" web/src/components/Calendar/WeekGrid/`.
