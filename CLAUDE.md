...
**Released:** `v0.4.11.6` (26.09.2026 — бот: цвет дня D3 в месяце и на «Сегодня», переключатели задач дня, подзадачи; API: `before_start` в `GET /api/day-tasks`; веб не менялся; прод и прод-бот на `22682e4`) · **Unreleased:**
...
- 🔴 **Бот v0.4.11.4 (20.09) — проход Дениса, дословно, остановлен на разделе 2** —
  ...
  повторяющиеся задачи были выпущены в релизных заметках, покрыты тестами и не существовали** —
  у API не было поля `rrule` ни в create, ни в update, а тесты сеяли колонку прямым INSERT'ом,
  минуя API. **Тест, который обходит дверь, не доказывает, что дверь есть.** Действующий
  чеклист — `docs/proverka-bota-v0.4.11.4-2026-09-20.md`; открытое оттуда: статистика
  с кнопками по периодам, задача↔событие, выключение повтора из бота, частота напоминаний
- 🔴 **Бот v0.4.11.3 (18.09) — проход Дениса с логами чата, дословно** —
  `ref/feedback/bot-proverka-v04113-otvet-denisa-2026-09-18.md`. Из него: snooze был сломан у
  всех (500 с миграции 000015, **и на проде тоже**), вторая напечатанная задача отвечала «Что-то
  пошло не так», «заметка» должна называться описанием, ❌ — отмена, 🗑 — удалить. Что по нему
  сделано за ночь — `docs/nochnoy-log-2026-09-18.md`, что перепроверить —
  `docs/proverka-nochnyh-pravok-2026-09-18.md`
- 🔴 **Бот v0.4.11.2 (17.09) — четыре прохода Дениса за вечер, дословно.** Первый проход,
  103 пункта — `ref/feedback/bot-proverka-v04112-otvet-denisa-2026-09-17.md`; второй —
  `ref/feedback/bot-povtor-otvet-denisa-2026-09-17.md`; третий —
  `ref/feedback/bot-prohod3-otvet-denisa-2026-09-17.md`; четвёртый, всё зелёное —
  `ref/feedback/bot-prohod4-otvet-denisa-2026-09-17.md`. Его правило из них: **каждое
  замечание — в сторону меньшего числа шагов**
- 🔴 **Первые внешние пользователи бота (16–17.09)** — `ref/feedback/bot-pervye-testery-2026-09-16-17.md`.
  Почему появился онбординг: язык не нашли, «слишком много всего», объяснять пришлось руками
...
- 🔴 **Приёмка перед релизом v0.4.11 (11.09) — самое свежее свидетельство** —
  ...
  редактор задачи модалкой поверх календаря · «общая» задача ничем не отмечена и не окрашена ·
  задача и созданное из неё событие не связаны
- **Что нашёл проход среза 1 (10.09)** —
  `ref/feedback/prohod-sreza-1-otvet-denisa-2026-09-10.md`. Бот прошёл целиком; смена календаря
  у события **не работает**, свайп по дням не подгружает события, перетаскивание задачи из
  сайдбара не работает
...
19. 🔴 **ОБА бота живут на nl-2 и НЕ выкатываются CI. Это делает «готово» про бота ложью
    ...
    - 🟢 **dev — `scripts/deploy-dev-bot.sh`** (22.09): те же шаги ниже плюс проверка «Bot started» в
      логах нового контейнера, три последних бэкапа `src.bak-*`. Прод — по-прежнему руками.
...
21. 🔴 **`PATCH /api/auth/me` ЗАМЕНЯЕТ весь blob `settings`, а не сливает его.**
    `auth/handlers.go:266` строит `settings = $N` — обычное присваивание, не `||`. Значит
    **любой клиент, пишущий часть настроек, стирает остальные.** Проверено на staging живьём
    18.08: засеяли 6 ключей (`header_variant`, `ui_scale`, `work_days`, `work_start`,
    `work_end`, `quiet_hours_start`) → отправили `{"settings":{"work_start":"08:00"}}` →
    в базе остался **один ключ**. HTTP 200, ни ошибки, ни предупреждения.
    - Веб выживает только потому, что держит объект целиком в памяти и шлёт его весь.
      Это не защита, а совпадение: любой новый клиент по умолчанию делает наоборот.
    - **Как правильно:** читать `GET /api/auth/me`, слить, отправить целиком — и **не писать
      вовсе, если чтение не удалось**, иначе слияние идёт поверх пустой карты и стирает всё.
      Образец — `bot/internal/api/client.go`, `MergeSettings` / `PatchSettings` + 4 теста.
    - ⚠ Чинить на сервере (`settings = COALESCE(settings,'{}') || $N`) заманчиво, но это меняет
      семантику для веба — удалить ключ станет невозможно. Решение за Денисом, не ночью.
...
## Как заканчивать ход (правило Дениса, 2026-07-19; поправлено по корню 20.09)

🔴 **Любая передача управления видна: строка статуса, а AskUserQuestion — когда есть настоящий
выбор** (варианты ведут к разной работе). Вопрос-формальность («готово? продолжаем?») — не вопрос:
60 из 1807 таких Денис закрыл Esc'ом. Дефект спрашивать открыто («что не так на экране?»), не
вариантом. Источник — корневой `E:\Projects\CLAUDE.md`, раздел «Asking the user».
⚠ До 22.09 здесь стояло «заканчивается AskUserQuestion. Без исключений» — корень отменил это
20.09, а эта строка продолжала приезжать в каждую сессию проекта и противоречить ему.

Откуда правило: старое говорило «когда нужен Денис ИЛИ когда готово» и пропускало третий случай —
...
Этот случай закрывает строка статуса.
...
3. **AskUserQuestion** — если выбор настоящий. Варианты ранжированы, рекомендованный первым,
   **каждый с размером в виде `~25 мин · ~120k`** (минуты агента · токены контекста), как в корне.
   ❌ «сессия», «2-3 часа» — не измеряются и выходили ~3× выше (Денис, 22.09).
