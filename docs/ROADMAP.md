...
| **P2** | Бот и уведомления | 🟡 **Шаги 1–7 из 10 собраны 2026-07-27**, на staging. Ждёт `SERVICE_TOKEN` |
| **P3** | Общие события и календари | ⬜ После P2 |

### P2 — что построено (шаги 1–7)
...
3. Шаги 8–9 — `<ReminderOffsets/>` в редакторе события, в задаче и в Settings. **Пока смещения
   можно задать только через API**, в UI их поставить негде.
4. ⚠️ Задачи из quick-add (P1) создаются с пустым `reminder_offsets` — применение
   пресета по умолчанию отложено до UI-шагов. То есть когда уведомления заработают,
   именно быстро созданные задачи в них не попадут, пока не сделан шаг 9.
