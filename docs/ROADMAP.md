<!-- паспорт: тип=документ | статус=действует | строк=399 | ~токенов=6583 | обновлён=по git -->
...
| **P2** | Бот и уведомления | 🟡 **9 шагов из 10** (шаг 7 закрыт 11.08, `38e6bec`). Остался шаг 10 |
| **P3** | Общие события и календари | ⬜ После P2 |

### P2 — что построено (шаги 1–9; шаг 7 закрыт 11.08)
...
✅ **Шаг 7 закрыт 11.08** (`38e6bec`): ручка `POST /api/svc/notifications/action` под тем же
service-токеном, inline-клавиатура по типу источника (задача → Готово/+10 мин, событие →
Понятно/+10 мин, дайджест → без кнопок: у сводки нет одного предмета действия).

Snooze пишет `minutes_before = -1` (sentinel «разовое, смещения нет»), время — в `remind_at`.
🔴 **Не NULL:** `idx_reminder_dedupe` объявлен `NULLS NOT DISTINCT`, и NULL в ключе столкнул бы
snooze с дайджестом. Повторный snooze **двигает** время (`DO UPDATE`), а conflict target повторяет
выражение `COALESCE(event_id, task_id)` — списком колонок в expression-индекс не попасть.

⚠️ **Спека сама себе противоречит:** §6 говорит `minutes_before = NULL`, §4 — sentinel `-1`.
Верен sentinel: NULL схема выразить не может без потери дедупликации.

Проверено на staging исполнением: 401 без токена · 401 с чужим · 400 на неизвестное действие ·
404 на чужое напоминание (с тем же 404 на несуществующее — намеренно неразличимо) · 200 на своё,
строка `-1 PENDING` с будущим `remind_at` · повторный snooze = **одна** строка со сдвинутым
временем · доставлено ботом со статусом **SENT**, то есть Telegram принял payload клавиатуры.

🔴 **Не проверено:** нажатие кнопки — это может сделать только Денис.

**Историческая справка:** цифра «1–9» родилась из коммита `7c282c5 docs: P2 steps 8-9 shipped` —
UI-шаги сделали вперёд седьмого, а документ записал это диапазоном.
