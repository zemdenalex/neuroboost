...
- [ ] Редактор события (`components/Calendar/EventEditor`) не закрывается по Escape: ни одного обработчика, кроме `TimeInput`. Правило проекта (`.claude/rules/react-frontend.md`): «Escape + клик мимо». Найдено e2e месяца 24.09; отдельный фикс с тестом · ~15 мин · ~30k

- [ ] `web/src/i18n/locales/{ru,en}/settings.json` — 10 длинных тире в текстах пользователю (не в `monthView`, тот чистый и под тестом). Правило Дениса — без длинных тире; поправить и распространить тест паритета на весь файл · ~10 мин · ~20k
