...
- [ ] Длинные тире в остальных текстах веба: `home` 9 · `onboarding` 24 · `reminders` 7 · `tasks` 2 · `tools` 6 (ru+en, 48 строк). Это переписывание копирайта (в русском тире часто грамматическое: «Календарь — это истина»), как в боте в D2; потом тест на все `locales/*.json` · ~30 мин · ~60k
