...
- [ ] 5. `components/MonthView/MonthView.tsx` — шапка (◀ ▶, «Сегодня», месяц словами), сетка 6×7, фильтр скрытых календарей, клетка A (`cells/ListCell.tsx`); сегодня выделено; квадрат дня. Заменить заглушки в `MonthView/` (удалить `monthview-context.txt` и пустые файлы) · ~25 мин · ~80k
- [ ] 6. `pages/Calendar/Calendar.tsx` — переключатель Неделя / Месяц (скрыт < 768 px), запоминание (шаг 3). Клик по дню → неделя с этим днём (смещение недели от сегодня); одиночный клик ждёт 250 мс (R6); двойной → `EventEditor` на 1 час с `work_start` (R7). Логику клика вынести в `lib/calendar/monthClick.ts` с тестом на таймер (vi.useFakeTimers) · ~25 мин · ~70k
- [ ] 7. Перетаскивание плашки (A) на другой день: pointer-drag с `DRAG_THRESHOLD_PX` (R8). Чистая функция `shiftByDays(startISO, endISO, days, tz)` в `lib/calendar/` — тест через переход DST (Europe/Berlin, последнее воскресенье октября): 10:00 остаётся 10:00. Повторяющееся → `withScope` (как в неделе) · ~30 мин · ~90k
- [ ] 8. i18n ru/en (`calendar.json`: month, week, today, more, day tasks off) + тест паритета, как `daytasks` · ~5 мин · ~10k
