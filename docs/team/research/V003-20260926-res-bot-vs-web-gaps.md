<!-- паспорт: тип=документ | статус=действует | строк=149 | ~токенов=4663 | обновлён=по git -->
...
**Сделано 26.09 (утро):** ✅ строка 2 повтор задачи (`a3b1f3e`) · ✅ 4 запланировать с выбором (`f371ab4`, `a42e7e7`) · ✅ 6 один язык (`96c0d82`, `c84b9d2`) · ✅ 7 подзадача из меню строки (`f01fe67`) · ✅ 9 дата «весь день» и несколько дней (`796c8de`) · ✅ 11 «каждые N» и «каждый год» (`5413110`). Попутно: запланированная задача остаётся в «К выполнению» (`6be5ba1`, как A15 бота); `GET /api/events` снова `[]`, а не `null` (`ef11fc0`); листы задач над нижней панелью телефона (`03a3b16`). Потом: ✅ 13 «долбить» (`7778dfb`) · ✅ 15 «📌 В задачи дня» из меню строки (`194f3ca`). 🟡 16 символ приоритета — в вебе это цветные точки у групп, не эмодзи в тексте: следовать ли выбору из бота — вкус, решает Денис. Открыто: 1, 3, 5, 16 (решения Дениса), 8, 10, 12, 14, 17, 18.
