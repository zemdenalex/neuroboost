...
- [x] MA3b Тема из `themeParams` (цвета Telegram) — **Денис 26.09: вариант 2, «всё из Telegram»**. `web/src/lib/theme/telegramPalette.ts` (+10 тестов): страница, панели, текст, подсказки, кнопки, ссылки из `themeParams`; пороги — текст/фон 4.5:1 иначе своя палитра, белый на кнопке 2:1 иначе свой синий. `blue-500/600/700` стали переменными (значения по умолчанию те же). e2e `mini-app.spec.ts`: ночная и светлая тема Telegram. Заодно иконки на круглых синих кнопках — `text-onaccent` (в светлой были чёрные)
  - 26.09: страница вариантов 0–3 (как есть · поверхности · всё · только акцент) на трёх примерах тем Telegram — https://claude.ai/artifact/GYZJSUPtrGaR6T5XNoVcyu ; ждёт выбора Дениса. Во всех вариантах цвета календарей не трогаются, вне Telegram ничего не меняется
